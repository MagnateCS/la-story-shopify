...
export class Slideshow extends Component {
    ...
    #setupContinuousAutoplay() {
        ...
        this.addEventListener('pointerdown', this.#handleContinuousPointerDown, {
            signal,
            passive: true
        });

        document.addEventListener('pointerup', this.#handleContinuousPointerUp, {
            signal,
            passive: true
        });

        document.addEventListener('pointercancel', this.#handleContinuousPointerUp, {
            signal,
            passive: true
        });

        this.addEventListener('wheel', this.#handleContinuousActivity, {
            signal,
            passive: true
        });

        this.addEventListener('keydown', this.#handleContinuousActivity, {
            signal
        });

        document.addEventListener('visibilitychange', this.#handleContinuousVisibility, {
            signal
        });
    ...
    #startContinuousAutoplay = () => {
        ...
        const animate = (timestamp) => {
            ...
            // Wait for images/layout if there is not yet enough content to scroll.
            // At the end, return to the first item and keep the autoplay alive.
            if (maxScroll > 0 && after >= maxScroll - 0.1) {
