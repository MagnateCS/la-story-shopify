...
import { Scroller, scrollIntoView } from '@theme/scrollinag';
...
export class Slideshow extends Component {
  ...
  async select(input, event, options = {}) {
    ...
    if (this.continuousAutoplay) this.#handleContinuousActivity();
  ...
  #continuousMaxScroll = 0;
  #continuousDirection = 1;
  #continuousLastSync = 0;
  #continuousStyle = null;
  get current() {
    return this.#current;
  }
  /**
  ...
  set current(value) {
    ...
    if (this.#current === value && this.#controlsInitialized) return;
  ...
  #setupSlideshow() {
    ...
    scheduler.schedule(() => {
      ...
      this.#resizeObserver = new ResizeObserver(async () => {
        ...
        if (this.continuousAutoplay) this.#measureContinuousScroll();
        if (this.continuousAutoplay && !this.#continuousPlaying && !this.#continuousPointerActive) {
          this.#startContinuousAutoplay();
        }
      });
  ...
  #setupContinuousAutoplay() {
    ...
    this.refs.scroller.addEventListener('load', this.#measureContinuousScroll, { signal, capture: true });
    // Start once layout provides scrollable content.
    requestAnimationFrame(() => this.#startContinuousAutoplay());
  }
  // Measure outside the animation loop. Images and container resizes refresh this cache.
  #measureContinuousScroll = () => {
    const { scroller } = this.refs;
    this.#continuousMaxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
  };
  /** Start a single time-based animation, preserving fractional movement. */
  #startContinuousAutoplay = () => {
    if (!this.continuousAutoplay || !this.#scroll || !this.isConnected) return;
    if (this.paused || this.disabled || this.#disabled || this.#continuousPlaying || this.#continuousResumeTimeout) return;
    if (document.hidden || this.#dragging || this.#continuousPointerActive || prefersReducedMotion()) return;
    const { scroller } = this.refs;
    this.#measureContinuousScroll();
    this.#continuousDirection = getComputedStyle(scroller).direction === 'rtl' ? -1 : 1;
    this.#continuousPosition = Math.max(0, this.#continuousDirection * scroller.scrollLeft);
    this.#continuousStyle = ['scroll-behavior', 'scroll-snap-type'].map((property) => ({
      property,
      value: scroller.style.getPropertyValue(property),
      priority: scroller.style.getPropertyPriority(property),
    }));
    ...
    const speed = this.continuousAutoplaySpeed;
    const animate = (timestamp) => {
      this.#continuousFrame = undefined;
      if (!this.#continuousPlaying || !this.isConnected) return;
      if (document.hidden || this.paused || this.disabled || this.#dragging || this.#continuousPointerActive) {
        this.#stopContinuousAutoplay();
        return;
      }
      if (!this.hasAttribute('in-viewport') || this.#continuousMaxScroll <= 0) {
        this.#continuousLastTime = undefined;
      } else if (this.#continuousLastTime === undefined) {
        this.#continuousLastTime = timestamp;
      } else {
        const elapsed = Math.min((timestamp - this.#continuousLastTime) / 1000, 0.05);
        this.#continuousLastTime = timestamp;
        // Never read the rounded browser position back into the accumulator.
        this.#continuousPosition += speed * elapsed;
        if (this.#continuousPosition > this.#continuousMaxScroll) {
          this.#continuousPosition = 0;
        }
        scroller.scrollLeft = this.#continuousDirection * this.#continuousPosition;
      }
      this.#continuousFrame = requestAnimationFrame(animate);
    };
    this.#continuousFrame = requestAnimationFrame(animate);
  };
  /** Stop movement and restore the original inline scroll styles. */
  #stopContinuousAutoplay = () => {
    this.#continuousPlaying = false;
    if (this.#continuousFrame !== undefined) cancelAnimationFrame(this.#continuousFrame);
    this.#continuousFrame = undefined;
    this.#continuousLastTime = undefined;
    if (this.#continuousStyle) {
      if (this.#scroll) this.#scroll.snap = true;
      for (const { property, value, priority } of this.#continuousStyle) {
        if (value) this.refs.scroller.style.setProperty(property, value, priority);
        else this.refs.scroller.style.removeProperty(property);
      }
      this.#continuousStyle = null;
    }
  };
   ...
   */
  #handleScroll = () => {
    // Native scroll events still drive accessibility and controls, but not at
    // animation-frame frequency while autoplay is moving.
    if (this.#continuousPlaying) {
      const now = performance.now();
      if (now - this.#continuousLastSync < 100) return;
      this.#continuousLastSync = now;
    }
    const previousIndex = this.#current;
  ...
  #handleMouseDown = (event) => {
    ...
    const onPointerMove = (event) => {
      ...
      if (!moved) {
        ...
        if (!this.continuousAutoplay) this.pause();
