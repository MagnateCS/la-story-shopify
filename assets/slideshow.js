...
export class Slideshow extends Component {
  ...
  #startContinuousAutoplay = () => {
    ...
    const animate = (timestamp) => {
      ...
      this.#continuousLastTime = timestamp;
      // Accumulate sub-pixel movement independently of browser rounding.
      const distance = this.continuousAutoplaySpeed * deltaSeconds;
      const { scroller } = this.refs;
      const maxScroll = Math.max(0, scroller.scrollWidth - scroller.clientWidth);
      continuousPosition = Math.min(continuousPosition + distance, maxScroll);
      scroller.scrollLeft = continuousPosition;
      ...
      if (maxScroll > 0 && continuousPosition >= maxScroll - 0.1) {
